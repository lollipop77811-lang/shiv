      ...
      <div className="container-x grid items-center gap-12 lg:grid-cols-2 lg:gap-16">
        ...
        <Reveal className="relative">
          <div className="relative">
            <div className="absolute -inset-3 rounded-[2.5rem] border border-azure-200/60" aria-hidden="true" />
            ...
            <div className="absolute -left-4 top-6 hidden items-center gap-2 rounded-full border border-border/60 bg-white/90 px-4 py-2.5 shadow-soft backdrop-blur sm:flex">
              ...
              <span className="text-xs font-bold text-navy-900">Your City — easy to reach</span>
            </div>
          </div>
        </Reveal>
          ...
          <Reveal delay={0.1}>
            ...
            <ul className="mt-7 grid gap-3 sm:grid-cols-2">
              {[
                ...
                <li key={point} className="flex items-center gap-2.5 text-sm font-semibold text-navy-800">
                  <CheckCircle2 className="h-5 w-5 shrink-0 text-azure-600" />
                  {point}
                </li>
              ))}
            </ul>
            <button
              ...
              className="group mt-8 inline-flex items-center gap-2 rounded-full bg-navy-900 px-7 py-3.5 text-sm font-bold text-white shadow-soft transition hover:bg-navy-800"
            >
              Learn More About Us
              <ArrowRight className="h-4 w-4 text-azure-300 transition-transform group-hover:translate-x-1" />
            </button>
          </Reveal>
        </div>
      </div>
            ...
            <StaggerItem key={d.id}>
              <article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-border/70 bg-white shadow-soft transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lift">
                <div className="relative overflow-hidden">
                  ...
                  />
                  <span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1.5 text-[0.68rem] font-extrabold uppercase tracking-[0.12em] text-azure-700 shadow-soft backdrop-blur">
                    {d.experience}
                  </span>
                </div>
                <div className="flex flex-1 flex-col p-5">
                  <h3 className="text-lg font-extrabold tracking-tight text-navy-900">{d.name}</h3>
                  <p className="mt-0.5 text-sm font-semibold text-azure-700">{d.specialization}</p>
                  <p className="mt-2 text-xs font-semibold leading-relaxed text-slate-500">{d.qualification}</p>
                  <div className="mt-4 flex flex-1 flex-col justify-end gap-2.5">
                    <button
                      onClick={() => nav.navigate({ view: "doctor", doctorId: d.id })}
                      className="w-full rounded-full border border-azure-200 bg-azure-50/60 px-4 py-2.5 text-sm font-bold text-azure-700 transition hover:border-azure-300 hover:bg-azure-50"
                    >
                      View Profile
                    </button>
                    <button
                      ...
                      className="w-full rounded-full bg-navy-900 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-navy-800"
                    >
                      Book Appointment
                    </button>
                  </div>
