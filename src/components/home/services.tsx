        ...
        <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          ...
          <Reveal delay={0.1} className="shrink-0">
            <button
              ...
              className="group inline-flex items-center gap-2 rounded-full border border-azure-200 bg-white px-6 py-3.5 text-sm font-bold text-navy-900 shadow-soft transition hover:border-azure-300 hover:bg-azure-50"
            >
              View All Treatments
              <ArrowRight className="h-4 w-4 text-azure-600 transition-transform group-hover:translate-x-1" />
            </button>
          </Reveal>
        </div>
          ...
          {TREATMENTS.map((t) => (
            <StaggerItem key={t.id}>
              <button
                ...
                className="group flex h-full w-full flex-col rounded-3xl border border-border/70 bg-white p-6 text-left shadow-soft transition-all duration-300 hover:-translate-y-1.5 hover:border-azure-200 hover:shadow-lift"
...
export function FindCare() {
  ...
  return (
    <section className="py-16 sm:py-20 lg:py-24" aria-labelledby="findcare-heading">
      <div className="container-x">
        <div className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-navy-900 via-navy-800 to-azure-800 shadow-lift">
          {/* decorative rings */}
          <svg
          ...
          <div className="relative p-8 sm:p-12 lg:p-16">
            <div className="max-w-2xl">
              <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.14em] text-azure-200">
              ...
              <p className="mt-3 text-base leading-relaxed text-azure-100/85">
                ...
                <button
                  ...
                  className={`rounded-full px-5 py-2.5 text-sm font-bold transition ${
                    tab === t.id
                      ...
                      : "text-azure-100 hover:text-white"
                  }`}
                >
                  {t.label}
                </button>
              ...
              {tab === "symptoms"
                ? SYMPTOMS.map((s) => (
                    <button
                      ...
                      className="group inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-5 py-3 text-sm font-semibold text-azure-50 transition hover:border-azure-300/50 hover:bg-white/15"
                    >
                      <span className="h-1.5 w-1.5 rounded-full bg-azure-300 transition group-hover:bg-white" />
                      {s.label}
                    </button>
                  ))
                : TREATMENTS.map((t) => (
                    <button
                      key={t.id}
                      onClick={() => nav.navigate({ view: "treatment", treatmentId: t.id })}
                      className="group inline-flex items-center gap-2.5 rounded-full border border-white/15 bg-white/5 px-5 py-3 text-sm font-semibold text-azure-50 transition hover:border-azure-300/50 hover:bg-white/15"
                    >
                      <t.icon className="h-4.5 w-4.5 text-azure-300 transition group-hover:text-white" />
                      {t.shortName}
                    </button>
                  ))}
            </div>

            <p className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-azure-200/80">
