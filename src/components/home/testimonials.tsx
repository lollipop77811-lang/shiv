...
function Stars({ n }: { n: number }) {
  return (
    <span className="inline-flex gap-0.5 text-gold-500" aria-label={`${n} star rating`}>
      ...
      <div className="container-x">
        ...
        <div className="mt-12 grid items-stretch gap-6 lg:grid-cols-[1.5fr_1fr]">
          ...
          <Reveal className="relative">
            <div className="relative h-full overflow-hidden rounded-[2rem] border border-border/70 bg-white p-8 shadow-soft sm:p-12">
              ...
              <AnimatePresence mode="wait" custom={dir}>
                ...
                >
                  <Stars n={item.rating} />
                  <blockquote className="mt-5 flex-1 text-xl font-semibold leading-relaxed text-brand-800 sm:text-2xl">
                    &ldquo;{item.quote}&rdquo;
                  </blockquote>
                  <figcaption className="mt-7 flex items-center gap-4">
                    <span className="grid h-12 w-12 place-items-center rounded-full bg-gradient-to-br from-jade-500 to-brand-800 text-base font-extrabold text-white">
                      {item.name.split(" ").map((w) => w[0]).join("")}
                    </span>
                    <span>
                      <span className="block text-sm font-extrabold text-brand-900">{item.name}</span>
                      <span className="block text-xs font-semibold text-jade-700">{item.treatment}</span>
                    </span>
                  </figcaption>
                </motion.figure>
              </AnimatePresence>
            ...
            <div className="mt-6 flex items-center justify-between">
                ...
                {TESTIMONIALS.map((_, i) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all ${
                      i === index ? "w-8 bg-jade-600" : "w-2 bg-jade-200 hover:bg-jade-300"
                    }`}
              ...
              <div className="flex gap-2.5">
                <button
                  ...
                  className="grid h-11 w-11 place-items-center rounded-full border border-border bg-white text-brand-900 shadow-soft transition hover:bg-jade-50"
                ...
                <button
                  onClick={() => go(index + 1, 1)}
                  className="grid h-11 w-11 place-items-center rounded-full bg-brand-900 text-white shadow-soft transition hover:bg-brand-800"
          ...
          <Reveal delay={0.1} className="h-full">
            <div className="flex h-full flex-col justify-between rounded-[2rem] bg-gradient-to-br from-brand-900 to-jade-800 p-8 text-white shadow-lift sm:p-10">
              <div>
                <p className="text-[0.7rem] font-extrabold uppercase tracking-[0.18em] text-jade-300">
                  Google Rating
                </p>
                ...
                <p className="mt-4 text-sm font-semibold leading-relaxed text-jade-100/85">
              ...
              <div className="mt-8 flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
                <BadgeCheck className="h-8 w-8 shrink-0 text-jade-300" />
                <p className="text-xs font-semibold leading-relaxed text-jade-100/80">
...
export function Blog() {
  const nav = useNav();
  const gradients = [
    "from-jade-500 to-brand-800",
    "from-jade-400 to-brand-700",
    "from-jade-300 to-jade-600",
    "from-jade-600 to-brand-900",
  ];
            ...
            <StaggerItem key={post.title}>
              ...
              >
                <div className={`relative flex h-36 items-end bg-gradient-to-br ${gradients[i % gradients.length]} p-5`}>
                  ...
                  <span className="rounded-full bg-white/90 px-3 py-1 text-[0.68rem] font-extrabold uppercase tracking-[0.12em] text-brand-900 backdrop-blur">
                    {post.category}
                  </span>
                </div>
                <div className="flex flex-1 flex-col p-6">
                  <p className="text-xs font-bold text-ink-400">{post.readTime}</p>
                  <h3 className="mt-2 text-base font-extrabold leading-snug tracking-tight text-brand-900 transition group-hover:text-jade-700">
                    {post.title}
                  </h3>
