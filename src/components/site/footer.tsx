...
export function Footer() {
  ...
  return (
    <footer className="bg-navy-950 pb-24 pt-16 text-azure-100/80 lg:pb-0">
      <div className="container-x">
          ...
          <div>
            ...
            <div className="mt-6 space-y-3 text-sm">
              <a href={CLINIC.phoneHref} className="flex items-center gap-3 transition hover:text-white">
                <Phone className="h-4 w-4 shrink-0 text-azure-400" /> {CLINIC.phoneDisplay}
              </a>
              <a href={`mailto:${CLINIC.email}`} className="flex items-center gap-3 transition hover:text-white">
                <Mail className="h-4 w-4 shrink-0 text-azure-400" /> {CLINIC.email}
              </a>
              ...
              {quickLinks.map((l) => (
                <li key={l.label}>
                  <button onClick={l.action} className="group inline-flex items-center gap-1.5 transition hover:text-white">
                    <span className="h-px w-3 bg-azure-500/60 transition-all group-hover:w-5 group-hover:bg-azure-300" aria-hidden="true" />
                    {l.label}
                  </button>
                </li>
              ))}
        ...
        {/* bottom bar */}
        <div className="mt-14 flex flex-col items-center justify-between gap-4 border-t border-white/10 py-7 text-xs font-medium text-azure-100/60 sm:flex-row">
      ...
      <button
        ...
        className="container-x group mb-8 flex items-center gap-1 text-xs font-bold text-azure-300 transition hover:text-white"
      >
