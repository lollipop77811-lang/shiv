...
export function Logo({ light = false }: { light?: boolean }) {
  ...
  return (
    ...
    >
      <span className="relative grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-azure-500 to-navy-900 shadow-soft">
        <svg viewBox="0 0 48 48" className="h-7 w-7" aria-hidden="true">
          ...
          <circle cx="24" cy="24" r="2.1" fill="#0a2540" />
      ...
      <span className="leading-tight">
        ...
        <span
          className={`block whitespace-nowrap text-[0.65rem] font-bold uppercase tracking-[0.22em] ${
            light ? "text-azure-200" : "text-azure-600"
          }`}
        >
          {CLINIC.tagline}
        </span>
  ...
  return (
    <>
      ...
      <div className="hidden bg-navy-900 text-white lg:block">
        <div className="container-x flex h-9 items-center justify-between text-xs font-medium text-azure-100/90">
        ...
        <div className="container-x flex h-[4.5rem] items-center justify-between gap-3">
                ...
                <button
                  ...
                  className={`whitespace-nowrap rounded-full px-2.5 py-2 text-[0.84rem] font-semibold transition ${
                    active
                      ...
                      : "text-slate-600 hover:bg-azure-50 hover:text-navy-900"
                  }`}
                >
                  {link.label}
                </button>
          ...
          <div className="flex items-center gap-2.5">
            <a
              ...
              className="hidden items-center gap-2 whitespace-nowrap rounded-full border border-azure-200 bg-azure-50/60 px-4 py-2.5 text-sm font-bold text-azure-700 transition hover:border-azure-300 hover:bg-azure-50 sm:inline-flex"
            >
            ...
            <button
              ...
              className="inline-flex items-center gap-2 whitespace-nowrap rounded-full bg-azure-600 px-4 py-2.5 text-sm font-bold text-white shadow-soft transition hover:bg-azure-700 hover:shadow-lift md:px-5"
            >
            ...
            <button
              className="grid h-11 w-11 place-items-center rounded-full border border-border bg-white text-navy-900 transition hover:bg-azure-50 xl:hidden"
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between border-b border-border px-5 py-4">
                ...
                <button
                  onClick={() => setOpen(false)}
                  className="grid h-10 w-10 place-items-center rounded-full bg-azure-50 text-navy-900"
              ...
              <nav className="flex-1 overflow-y-auto px-3 py-4" aria-label="Mobile">
                {NAV_LINKS.map((link) => (
                  <button
                    key={link.label}
                    onClick={() => handleLink(link)}
                    className="flex w-full items-center justify-between rounded-2xl px-4 py-3.5 text-left text-base font-semibold text-navy-900 transition hover:bg-azure-50"
                  >
                    {link.label}
              ...
              <div className="space-y-3 border-t border-border p-5">
                <a
                  href={CLINIC.phoneHref}
                  className="flex w-full items-center justify-center gap-2 rounded-full border border-azure-200 bg-azure-50 px-4 py-3 text-sm font-bold text-azure-700"
                >
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-full bg-azure-600 px-4 py-3 text-sm font-bold text-white shadow-soft"
                >
