    ...
    return (
      <section className="bg-mist py-16 sm:py-24">
        <div className="container-x max-w-2xl">
          ...
          >
            <div className="bg-gradient-to-br from-plum-900 to-orchid-700 px-8 py-12 text-center">
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
              ...
              <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-orchid-100/85">
            ...
            <div className="p-8 sm:p-10">
              <dl className="grid gap-3 rounded-2xl bg-mist p-6 text-sm">
                {[
                  ...
                  <div key={k} className="flex flex-wrap justify-between gap-2">
                    <dt className="font-bold text-plum-900">{k}</dt>
                    <dd className="text-right text-ink-500">{v}</dd>
                  </div>
                ))}
              </dl>
              <p className="mt-5 rounded-2xl border border-orchid-200 bg-orchid-50 p-4 text-xs font-semibold leading-relaxed text-orchid-700">
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                <button
                  ...
                  className="flex-1 rounded-full bg-plum-900 px-6 py-3.5 text-sm font-bold text-white transition hover:bg-plum-800"
                >
                  Back to Home
                </button>
                <button
                  onClick={reset}
                  className="flex-1 rounded-full border border-orchid-200 bg-orchid-50 px-6 py-3.5 text-sm font-bold text-orchid-700 transition hover:bg-orchid-100"
                >
                  Book Another
                </button>
              </div>
    ...
    <section className="bg-mist py-12 sm:py-16">
      <div className="container-x">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <h1 className="mt-5 text-3xl font-extrabold tracking-tight text-plum-900 sm:text-4xl lg:text-[2.75rem]">
          ...
          <Reveal>
            <aside className="rounded-3xl border border-border/70 bg-white p-7 shadow-soft lg:sticky lg:top-24">
                    ...
                    <li key={label} className="flex shrink-0 items-center gap-3 lg:w-full">
                      <span
                        className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-xs font-extrabold transition-all ${
                            ...
                            : active
                              ? "bg-plum-900 text-white ring-4 ring-orchid-100"
                              : "bg-mist text-ink-400"
                        }`}
                      >
                      ...
                      <span
                        className={`whitespace-nowrap text-sm font-bold lg:whitespace-normal ${
                          active ? "text-plum-900" : done ? "text-orchid-700" : "text-ink-400"
                        }`}
                      >
                        {label}
                      </span>
              ...
              <div className="mt-7 hidden rounded-2xl bg-mist p-5 lg:block">
                <p className="text-[0.65rem] font-extrabold uppercase tracking-[0.16em] text-ink-400">
                  Your Selection
                </p>
                <ul className="mt-3 space-y-2 text-sm font-semibold text-plum-800">
            ...
            <div className="rounded-3xl border border-border/70 bg-white p-7 shadow-soft sm:p-9">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  {step === 0 && (
                    <div>
                      <h2 className="text-xl font-extrabold tracking-tight text-plum-900">
                        Select Department
                      </h2>
                        ...
                        {DEPARTMENTS.map((dep) => (
                          <button
                            ...
                            className={`group flex items-center gap-3 rounded-2xl border p-4 text-left transition-all ${
                              department === dep.id
                                ? "border-orchid-600 bg-orchid-50 ring-2 ring-orchid-200"
                                : "border-border bg-white hover:border-orchid-300 hover:bg-orchid-50/40"
                            }`}
                          >
                            <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl transition ${
                              department === dep.id ? "bg-orchid-600 text-white" : "bg-mist text-orchid-600"
                            }`}>
                              <dep.icon className="h-5 w-5" />
                            </span>
                            <span className="text-sm font-extrabold text-plum-900">{dep.name}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                      ...
                      <div className="mt-6 grid gap-3 sm:grid-cols-2">
                        <button
                          ...
                          className={`flex items-center gap-3.5 rounded-2xl border p-4 text-left transition-all ${
                            doctorId === "any"
                              ? "border-orchid-600 bg-orchid-50 ring-2 ring-orchid-200"
                              : "border-border bg-white hover:border-orchid-300"
                          }`}
                        >
                          <span className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-gradient-to-br from-orchid-400 to-plum-800 text-sm font-extrabold text-white">
                            Any
                          </span>
                          <span>
                            <span className="block text-sm font-extrabold text-plum-900">No Preference</span>
                            <span className="text-xs font-semibold text-ink-500">Matched to earliest slot</span>
                          </span>
                        </button>
                        {doctors.map((doc) => (
                          <button
                          ...
                          >
                            ...
                            />
                            <span>
                              <span className="block text-sm font-extrabold text-plum-900">{doc.name}</span>
                              <span className="block text-xs font-semibold text-orchid-700">{doc.specialization}</span>
                            </span>
                          </button>
                        ))}
                        {doctors.length === 0 && (
                          <p className="rounded-2xl bg-mist p-4 text-sm font-semibold text-ink-500">
                  ...
                  {step === 2 && (
                    <div>
                      <div className="flex items-center justify-between">
                        <div>
                          <h2 className="text-xl font-extrabold tracking-tight text-plum-900">Select Date</h2>
                          <p className="mt-1.5 text-sm text-ink-500">Sundays and past dates are not bookable.</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => setMonthOffset((m) => Math.max(0, m - 1))}
                            disabled={monthOffset === 0}
                            className="grid h-10 w-10 place-items-center rounded-full border border-border transition enabled:hover:bg-orchid-50 disabled:opacity-30"
                      ...
                      <p className="mt-6 text-center text-base font-extrabold uppercase tracking-[0.14em] text-plum-900">
                        {MONTH_FMT.format(new Date(viewYear, viewMonth, 1))}
                      </p>
                      <div className="mt-4 grid grid-cols-7 gap-1.5 text-center">
                        {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d) => (
                          <span key={d} className="py-2 text-[0.7rem] font-extrabold uppercase tracking-wider text-ink-400">
                            {d}
                          </span>
                        ))}
                        {grid.map((cell, i) => {
                            ...
                            <button
                              ...
                              className={`aspect-square rounded-xl text-sm font-bold transition-all ${
                                !cell.inMonth
                                  ? "opacity-0"
                                  : selected
                                    ? "scale-105 bg-orchid-600 text-white shadow-soft"
                                    : cell.disabled
                                      ? "text-orchid-300"
                                      : "text-plum-800 hover:bg-orchid-50 hover:ring-2 hover:ring-orchid-100"
                  ...
                  {step === 3 && (
                    <div>
                      <div className="flex items-center gap-3">
                        <span className="grid h-11 w-11 place-items-center rounded-2xl bg-orchid-50 text-orchid-600">
                          <Clock className="h-5.5 w-5.5" />
                        </span>
                        <div>
                          <h2 className="text-xl font-extrabold tracking-tight text-plum-900">Select Time</h2>
                          <p className="mt-0.5 text-sm font-semibold text-orchid-700">
                          ...
                          return (
                            <button
                              ...
                              className={`rounded-xl px-2 py-3 text-sm font-bold transition-all ${
                                booked
                                  ? "cursor-not-allowed bg-mist text-orchid-300 line-through"
                                  : time === slot
                                    ? "bg-orchid-600 text-white shadow-soft"
                                    : "bg-mist text-plum-800 hover:bg-orchid-50 hover:ring-2 hover:ring-orchid-100"
                              }`}
                            >
                              {slot}
                            </button>
                  ...
                  {step === 4 && (
                    <div>
                      <h2 className="text-xl font-extrabold tracking-tight text-plum-900">Patient Details</h2>
                      <p className="mt-1.5 text-sm text-ink-500">
                        We will call to confirm your appointment.
                      </p>
                      <div className="mt-6 grid gap-4 sm:grid-cols-2">
                        <label className="block">
                          <span className="mb-1.5 flex items-center gap-1.5 text-xs font-extrabold uppercase tracking-[0.12em] text-plum-900">
                            <User className="h-3.5 w-3.5 text-orchid-600" /> Full Name *
                          </span>
                          <input
                            value={name}
                            onChange={(e) => setName(e.target.value)}
                            placeholder="e.g. Priya Nair"
                            className="w-full rounded-2xl border border-border bg-mist/60 px-4.5 py-3.5 text-sm font-semibold outline-none transition focus:border-orchid-400 focus:bg-white focus:ring-2 focus:ring-orchid-100"
                          />
                        </label>
                        <label className="block">
                          <span className="mb-1.5 flex items-center gap-1.5 text-xs font-extrabold uppercase tracking-[0.12em] text-plum-900">
                            <Phone className="h-3.5 w-3.5 text-orchid-600" /> Phone Number *
                          </span>
                          <input
                            value={phone}
                        ...
                        <label className="block sm:col-span-2">
                          <span className="mb-1.5 flex items-center gap-1.5 text-xs font-extrabold uppercase tracking-[0.12em] text-plum-900">
                            <Mail className="h-3.5 w-3.5 text-orchid-600" /> Email (optional)
                          </span>
                          <input
                            value={email}
                        ...
                        <label className="block sm:col-span-2">
                          <span className="mb-1.5 block text-xs font-extrabold uppercase tracking-[0.12em] text-plum-900">
                            Notes (optional)
                          </span>
                          <textarea
                            value={notes}
                            ...
                            rows={3}
                            className="w-full resize-none rounded-2xl border border-border bg-mist/60 px-4.5 py-3.5 text-sm font-semibold outline-none transition focus:border-orchid-400 focus:bg-white focus:ring-2 focus:ring-orchid-100"
                          />
                        </label>
                      </div>
                      ...
                      <div className="mt-6 rounded-2xl bg-mist p-6">
                        ...
                        <ul className="mt-3 space-y-2 text-sm font-semibold text-plum-800">
                          <li className="flex justify-between gap-3"><span className="text-ink-400">Department</span>{dept?.name}</li>
                          <li className="flex justify-between gap-3"><span className="text-ink-400">Doctor</span>{doctor ? doctor.name : "Any available"}</li>
                          <li className="flex justify-between gap-3"><span className="text-ink-400">Date</span>{date ? DAY_FMT.format(date) : "—"}</li>
                          <li className="flex justify-between gap-3"><span className="text-ink-400">Time</span>{time}</li>
                        </ul>
                      </div>
                    </div>
                  )}
              ...
              <div className="mt-9 flex items-center justify-between gap-4 border-t border-border pt-6">
                <button
                  ...
                  disabled={step === 0}
                  className="inline-flex items-center gap-2 rounded-full border border-border px-6 py-3.5 text-sm font-bold text-plum-900 transition enabled:hover:bg-orchid-50 disabled:invisible"
                >
                ...
                {step < 4 ? (
                  <button
                    ...
                    className={`group inline-flex items-center gap-2 rounded-full px-8 py-3.5 text-sm font-extrabold transition-all ${
                      canContinue
                        ? "bg-orchid-600 text-white shadow-soft hover:bg-orchid-700"
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-2 rounded-full px-8 py-3.5 text-sm font-extrabold transition-all ${
                      canContinue
                        ? "bg-plum-900 text-white shadow-soft hover:bg-plum-800"
                        : "cursor-not-allowed bg-mist text-ink-400"
                    }`}
                  >
